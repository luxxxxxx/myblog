...
router.get("/", (req, res) => {
    ...
    if (type === 'techo') { // 如果栏目为科技页面
        queryCondition = 'where a_tags like "%科技%" or a_tags like "%宇宙%" or a_tags like "%探索%" or a_tags like "%科普%"'; //限定标签 
    } else if (type === 'anime') {
                ...
                mysql({
                    ...
                    callback: (err, info) => {
                        if (!err) {
                            ...
                            res.locals.result = '500 服务器发生了一个无法预料的问题,请联系网站管理员，QQ 981236133,错误信息：ERR987234';
                            res.status(500).render('500');
                        }
                    }
                })
            } else {
                res.locals.result = '500 服务器发生了一个无法预料的问题,请联系网站管理员，QQ 981236133,错误信息：ERR987222 <br />' + err;
                res.status(500).render('500');
            }
        }
    })
